...
export type TransferWorkerCommand =
  ...
  | { type: 'chunk'; transferId: string; slot: number; buffer: ArrayBuffer; byteOffset: number; byteLength: number }
...
export type TransferWorkerEvent =
  ...
  | { type: 'chunk-written'; transferId: string; slot: number; buffer: ArrayBuffer; byteOffset: number; byteLength: number }
  | { type: 'terminal'; transferId: string; status: 'completed' | 'cancelled' | 'failed'; message?: string };
...
export function postChunk(port: MessagePort, command: Extract<TransferWorkerCommand, { type: 'chunk' }>): void {
