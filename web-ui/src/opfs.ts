...
export async function openOpfsSink(name: string, expectedSize: number): Promise<OpfsSink> {
  ...
  try { writable = await file.createWritable({ keepExistingData: false }); }
  catch (error) {
  ...
  function cleanupPartial(): Promise<void> {
    return cleanup ??= (async () => {
      try { if (!writerClosed) await writable.abort(); }
      finally { await staging.removeEntry(entryName); }
    })();
  }

  function assertOpen(): void {
    if (cancelled || failed || committed) throw new Error('OPFS sink is closed');
  }
  ...
  async function guarded<T>(action: () => Promise<T>): Promise<T> {
    try { assertOpen(); return await action(); }
    catch (error) {
      accepting = false;
      failed = true;
  ...
  return {
    write(chunk: Uint8Array): Promise<void> {
      if (!accepting) return Promise.reject(new Error('OPFS sink is closed'));
    ...
    commit(): Promise<OpfsReceivedItem> {
      ...
      return enqueue(() => guarded(async () => {
        ...
        if (!file.move) throw new Error('OPFS move is unavailable');
