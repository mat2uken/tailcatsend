...
export function placeAnchor(
  ...
  const canFitBelow = viewportTop + viewportHeight - anchorRect.bottom >= floatingRect.height + offset;
  ...
  const preferredX = placement.endsWith('end')
    ...
    : anchorRect.left;
  const x = Math.max(viewportLeft + padding, Math.min(preferredX, viewportLeft + viewportWidth - floatingRect.width - padding));
  const clampedY = Math.max(viewportTop + padding, Math.min(y, viewportTop + viewportHeight - floatingRect.height - padding));
