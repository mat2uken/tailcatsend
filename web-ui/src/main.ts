...
const { a, button, div, footer, header, h1, h2, input, label, main, p, progress, section, span, textarea } = van.tags;

const isJapanese = navigator.language.toLowerCase().startsWith('ja');
const uiText = isJapanese
  ? {
      app: 'Ponlet', cancel: 'キャンセル', send: '送信', chooseFile: 'ファイルを選択',
      copy: 'コピー', share: '共有', save: '保存', invitation: '招待URLを貼り付け', connect: '接続',
      createInvite: '招待を作成', disconnect: '切断', settings: '設定', close: '閉じる',
      settingsDescription: 'この移行用UIの設定画面は準備中です。', message: 'メッセージ',
      transfer: '転送', messages: 'メッセージ', preparing: '安全なP2P通信を準備中…', connected: '接続済み',
      waiting: '相手を待機中', peer: '相手', saved: '招待URLをコピー',
    }
  : {
      app: 'Ponlet', cancel: 'Cancel', send: 'Send', chooseFile: 'Choose file', copy: 'Copy',
      share: 'Share', save: 'Save', invitation: 'Paste invitation URL', connect: 'Connect',
      createInvite: 'Create invite', disconnect: 'Disconnect', settings: 'Settings', close: 'Close',
      settingsDescription: 'Settings are not available in this migration UI yet.', message: 'Message',
      transfer: 'Transfer', messages: 'Messages', preparing: 'Preparing secure P2P network…', connected: 'Connected',
      waiting: 'Waiting for peer', peer: 'Peer', saved: 'Copy invitation',
    };

const backend: PonletBackend = createBackend();
const snapshot = van.state(initialSnapshot());
const messages = van.state<Message[]>([]);
...
async function perform(action: () => Promise<void>): Promise<void> {
  if (operationBusy.val) return;
  operationBusy.val = true;
  try { await run(action); }
  finally { operationBusy.val = false; }
}

window.addEventListener('pagehide', (event) => {
  if (!event.persisted) void run(() => session.dispose());
});
...
const settingsButton = button({ class: 'icon-button', type: 'button', 'aria-label': uiText.settings }, '⚙');
...
van.derive(() => {
  ...
  status.textContent = value.error ?? (value.state === 'connected' ? uiText.connected : value.state === 'awaiting-peer' ? uiText.waiting : uiText.preparing);
  ...
  copyTextButton.disabled = shareTextButton.disabled = saveTextButton.disabled = !lastReceivedText.val;
  if (value.transfer) {
...
van.derive(() => {
  log.replaceChildren(...messages.val.map((message) => p({ class: 'message' }, `${message.incoming ? '[Peer]' : '[Me]'}: ${message.text}`)));
});
...
sendButton.addEventListener('click', () => void perform(async () => {
  const text = textDraft.val;
  if (!text.trim() || !snapshot.val.canSend) return;
...
fileInput.addEventListener('change', () => {
  ...
  fileInput.value = '';
  if (files.length && snapshot.val.canSend) void perform(() => backend.sendFiles(files));
});
cancelButton.addEventListener('click', () => {
  const id = snapshot.val.transfer?.id;
  if (id) void run(() => backend.cancelTransfer(id));
});
joinInput.addEventListener('input', () => (joinDraft.val = joinInput.value));
connectButton.addEventListener('click', () => {
  const invite = joinDraft.val.trim();
  if (invite) void perform(() => backend.join(invite));
});
...
copyTextButton.addEventListener('click', () => {
  if (lastReceivedText.val) void run(() => backend.copyText(lastReceivedText.val));
});
shareTextButton.addEventListener('click', () => {
  if (lastReceivedText.val) void run(() => backend.shareText(lastReceivedText.val));
});
saveTextButton.addEventListener('click', () => {
  if (lastReceivedText.val) void run(() => backend.saveText(lastReceivedText.val));
});
const popover = div({ id: 'invite-popover', popover: 'auto', class: 'popover' }, isJapanese ? '招待URLをコピーしました' : 'Invitation copied');
...
document.body.append(
  header({ class: 'topbar' }, h1(uiText.app), div({ class: 'topbar-actions' }, peer, settingsButton)),
  main(
    { class: 'shell' },
    section({ class: 'connection-card' }, status, div({ class: 'connection-actions' }, joinInput, connectButton, createButton, disconnectButton), invite),
    section({ class: 'transfer-card' }, h2(uiText.transfer), div({ class: 'transfer-row' }, fileButton, fileInput, transferName, transferProgress, transferBytes, cancelButton)),
    section(
      { class: 'chat-card' },
      h2(uiText.messages),
      log,
