...
export type { BackendEvent, BackendSnapshot, PonletBackend, SessionState } from './api/application-api';

export type NativeBridge = Partial<PonletBackend>;

declare global {
  interface Window { __ponletBackend?: NativeBridge }
}
...
function unavailableBackend(): PonletBackend {
  ...
  return {
    ...
    subscribe: () => () => undefined,
    createInvite: unavailable, join: unavailable, sendText: unavailable,
    sendFiles: unavailable, cancelTransfer: unavailable, disconnect: unavailable,
    copyText, shareText, saveText, dispose: async () => undefined,
  };
}
...
export function createBackend(bridge: NativeBridge | undefined = window.__ponletBackend): PonletBackend {
  if (!bridge) return unavailableBackend();
  const required = ['snapshot', 'subscribe', 'createInvite', 'join', 'sendText',
    'sendFiles', 'cancelTransfer', 'disconnect', 'dispose'] as const;
  if (required.some((method) => typeof bridge[method] !== 'function')) return unavailableBackend();
  ...
  return {
    ...
    subscribe: (listener) => {
      ...
      if (typeof unsubscribe !== 'function') throw new Error('Backend subscription has no cleanup');
      return unsubscribe;
    },
