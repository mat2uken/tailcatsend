...
/*
 * Draft C ABI for TailSend's native Tailcat transport.
 *
 * This header is a design input, not a frozen ABI. Phase 0 must prove the
 * target-specific Go build modes and then record any changes in an ADR.
 *
 * Ownership rules:
 * - All handles are opaque process-local integers owned by the bridge.
 * - Caller-owned input memory is valid only for the duration of the call.
 * - Output strings are copied into caller-provided buffers.
 * - close functions are idempotent.
 * - tc_wait_event must not be called concurrently by multiple consumers.
 */
 ...
 * Exact semantics must be documented by the implementation.
 */
