...
TailSend（製品名 Ponlet）の iOS・macOS 版 App Store Connect 提出時のメタデータです。[fastlane deliver](https://docs.fastlane.tools/actions/deliver/) が読み込む `metadata/` と、App Store Connect の設問へ転記した補助資料 `review_information/` で構成しています。

この内容とスクリーンショットは 2026-09-24 に提出した Ponlet 1.0.14 の記録です。現在のソース（1.0.17）向けの提出物・検証済み回答ではありません。とくに審査回答に残る Remote Config の記述は 1.0.14 当時のもので、1.0.17 には当てはまりません。提出経緯と未確認事項は [`docs/APPSTORE_SUBMISSION_CHECKLIST.md`](../docs/APPSTORE_SUBMISSION_CHECKLIST.md) を参照してください。
...
この配下は fastlane deliver が読み込まない 1.0.14 提出時の回答資料です。現在のソースを提出する場合は、各設問を再確認してください。
