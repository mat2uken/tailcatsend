...
3. `NSBonjourServices`（`_tailsend._tcp` / `_tailsend._udp`）に対応する実装はコードにないことを確認済み。ユーザー方針により `project.yml` と `tailsend-tauri_iOS/Info.plist` から削除済み（2026-09-23）。`NSLocalNetworkUsageDescription` は実使用があるため残している。
