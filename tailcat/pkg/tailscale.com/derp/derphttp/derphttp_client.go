...
func (c *Client) connect(ctx context.Context, caller string) (client *derp.Client, connGen int, err error) {
	...
	default:
		...
		c.logf("%s: dialRegion for derp-%d result: ok=%v, err=%v", caller, reg.RegionID, err == nil, err)
		idealNodeInRegion = err == nil && reg.Nodes[0] == node
	}
	if err != nil {
		c.logf("%s: tcp connection failed: %v", caller, err)
		return nil, 0, err
	}
	...
	if c.useHTTPS() {
		...
		if err := tlsConn.Handshake(); err != nil {
			c.logf("%s: TLS handshake with %v failed: %v", caller, node.HostName, err)
			return nil, 0, err
		}
		c.logf("%s: TLS handshake with %v succeeded!", caller, node.HostName)
...
const dialNodeTimeout = 15 * time.Second
...
func (c *Client) dialNode(ctx context.Context, n *tailcfg.DERPNode) (net.Conn, error) {
	...
	if runtime.GOOS != "android" && shouldDialProto(n.IPv6, netip.Addr.Is6) {
	...
	for {
		...
		case res := <-resc:
			...
			c.logf("dialNode: dial error: %v", res.err)
			if firstErr == nil {
				firstErr = res.err
			}
			if nwait == 0 {
...
func parseMetaCert(certs []*x509.Certificate) (serverPub key.NodePublic, serverProtoVersion int) {
	for _, cert := range certs {
		...
		if pubHex, ok := strings.CutPrefix(cert.Subject.CommonName, derpconst.MetaCertCommonNamePrefix); ok {
			...
			if err == nil && cert.SerialNumber != nil && cert.SerialNumber.BitLen() <= 8 { // supports up to version 255
