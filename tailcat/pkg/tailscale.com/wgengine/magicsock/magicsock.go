...
func (c *Conn) updateEndpoints(why string) {
	...
	if c.noV4Send.Load() && runtime.GOOS != "js" && runtime.GOOS != "android" && !c.onlyTCP443.Load() && !hostinfo.IsInVM86() {
		c.mu.Lock()
		closed := c.closed
		c.mu.Unlock()
		if !closed {
...
func (c *Conn) updateNetInfo(ctx context.Context) (*netcheck.Report, error) {
	...
	if runtime.GOOS == "android" {
		...
		c.lastNetCheckReport.Store(rep)
		c.noV4.Store(false)
		c.noV4Send.Store(false)
		return rep, nil
	}
	...
	report, err := c.netChecker.GetReport(ctx, dm, &netcheck.GetReportOpts{
		...
		GetLastDERPActivity: func(reg tailcfg.DERPRegionID) time.Time {
			if c.health != nil {
				return c.health.GetDERPRegionReceivedTime(reg)
			}
			return time.Time{}
		},
...
func (c *Conn) determineEndpoints(ctx context.Context) ([]tailcfg.Endpoint, error) {
	...
	if runtime.GOOS == "js" || runtime.GOOS == "android" {
		return []tailcfg.Endpoint{
			{
				Addr: netip.MustParseAddrPort("127.0.0.1:45204"),
	...
	if localAddr := c.pconn4.LocalAddr(); localAddr != nil {
...
func (c *Conn) networkDown() bool {
	if runtime.GOOS == "android" || envknob.AssumeNetworkUp() || (testenv.InTest() && !c.checkNetworkUpDuringTests) {
		return false
	}
...
func (c *Conn) receiveIPv4() conn.ReceiveFunc {
	var hStats *health.ReceiveFuncStats
	if c.health != nil {
		hStats = c.health.ReceiveFuncStats(health.ReceiveIPv4)
	}
	return c.mkReceiveFunc(&c.pconn4, hStats,
...
func (c *Conn) receiveIPv6() conn.ReceiveFunc {
	var hStats *health.ReceiveFuncStats
	if c.health != nil {
		hStats = c.health.ReceiveFuncStats(health.ReceiveIPv6)
	}
	return c.mkReceiveFunc(&c.pconn6, hStats,
