...
func (c *Conn) derpWriteChanForRegion(regionID tailcfg.DERPRegionID, peer key.NodePublic) chan derpWriteRequest {
	if c.networkDown() {
		c.logf("derpWriteChanForRegion(%d): networkDown", regionID)
		return nil
	}
	...
	defer c.mu.Unlock()
	if !c.wantDerpLocked() {
		c.logf("derpWriteChanForRegion(%d): !wantDerpLocked", regionID)
		return nil
	}
	if c.closed {
		c.logf("derpWriteChanForRegion(%d): closed", regionID)
		return nil
	}
	if c.derpMap == nil {
		c.logf("derpWriteChanForRegion(%d): derpMap == nil", regionID)
		return nil
	}
	if c.derpMap.Regions[regionID] == nil {
		c.logf("derpWriteChanForRegion(%d): derpMap.Regions[%d] == nil (have %d regions)", regionID, regionID, len(c.derpMap.Regions))
		return nil
	}
	if c.privateKey.IsZero() {
...
func (c *Conn) runDerpReader(ctx context.Context, regionID tailcfg.DERPRegionID, dc *derphttp.Client, wg *syncs.WaitGroupChan, startGate <-chan struct{}) {
	...
	defer func() {
		if c.health != nil {
			...
			c.health.SetDERPRegionHealth(regionID, "")
		}
	}()
