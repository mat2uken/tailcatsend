...
//
//   swift scripts/generate_desktop_icons.swift [元絵 PNG] [--targets=tauri,desktop,web]
//
// 出力（--targets で省略時は全部生成）:
//   tauri:
//     apps/tauri/icons/icon.png  512x512（角丸の外側は透過、内側は不透明）
//     apps/tauri/icons/icon.ico  16 / 32 / 48 / 64 / 128 / 256 の複数サイズ
//   desktop:
//     apps/desktop/icon.png      512x512（tauri の icon.png と同じ形）
//     apps/desktop/AppIcon.icns  16 / 32 / 64 / 128 / 256 / 512 / 1024
...
//     dist/assets/icon.png       512x512
//     dist/assets/icon_512.png   512x512
//
...
var targets = Set(["tauri", "desktop", "web"])
...
if targets.contains("desktop") {
    print("\n🍎 [2/3] apps/desktop/icon.png (512x512) + AppIcon.icns (16..1024)...")
    guard let icon512Png = pngEntry(size: 512) else {
        print("❌ Failed to encode icon.png")
        exit(1)
    }
    try save(icon512Png, "apps/desktop/icon.png")

    let iconsetURL = FileManager.default.temporaryDirectory.appendingPathComponent("Ponlet.iconset")
    ...
    let icnsURL = repoRoot.appendingPathComponent("apps/desktop/AppIcon.icns")
    let process = Process()
...
// ==============================================================================
// 3. dist/ (Web 用 favicon 類と assets)
//    favicon.ico は 16/32/48 の複数サイズ ICO（32px は BMP + AND マスク）。
// ==============================================================================
if targets.contains("web") {
    print("\n🌐 [3/3] dist/ web favicons (32 / 180 / ico 16+32+48) and assets (512)...")
    ...
    guard let icon512 = pngEntry(size: 512) else {
        print("❌ Failed to encode assets/icon.png")
        exit(1)
    }
    try save(icon512, "dist/assets/icon.png")
    try save(icon512, "dist/assets/icon_512.png")
}
