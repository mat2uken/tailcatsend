...
# The iOS target is validated even when generating only the macOS scheme.
# Its frontend asset directory is created by the mobile build in a clean checkout.
...
# The distribution check consumes a preflight archive as well as the app.
# Reuse the slices compiled by the build phase instead of rebuilding Go/Rust.
archive="${derived_data}/Ponlet-preflight.xcarchive"
PONLET_MACOS_PREBUILT=1 xcodebuild -project "${project_dir}/tailsend-tauri.xcodeproj" \
  -scheme tailsend-tauri_macOS -configuration "${configuration}" \
  -derivedDataPath "${derived_data}" -destination 'generic/platform=macOS' \
  -archivePath "${archive}" "ARCHS=${archs}" CODE_SIGNING_ALLOWED=NO archive
test -d "${archive}/Products/Applications/Ponlet.app"
...
printf 'macOS preflight archive: %s\n' "${archive}"
