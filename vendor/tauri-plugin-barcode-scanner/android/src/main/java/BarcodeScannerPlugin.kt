...
class BarcodeScannerPlugin(private val activity: Activity) : Plugin(activity) {
    ...
    private fun setupCamera(cameraDirection: String, windowed: Boolean, generation: Long) {
        activity
            .runOnUiThread {
                ...
                cameraProviderFuture.addListener(
                    {
                        // The provider future may finish after cancel or a new scan.
                        if (scanSession.isCurrent(generation) &&
                            this.cameraProviderFuture === cameraProviderFuture
                        ) {
                            try {
                                ...
                                destroy(generation)?.reject(error.message ?: "Cannot prepare camera")
                            }
                        }
                    },
    ...
    private fun bindPreview(cameraProvider: ProcessCameraProvider, cameraDirection: Int, generation: Long) {
        activity
            .runOnUiThread {
                if (!scanSession.isCurrent(generation)) return@runOnUiThread
                ...
                imageAnalysis.setAnalyzer(
                    ContextCompat.getMainExecutor(activity),
                    { image -> analyze(image, generation) }
                )
                ...
                try {
                    ...
                    destroy(generation)?.reject(error.message ?: "Cannot bind camera")
                }
            }
    }
    ...
    private fun destroy(generation: Long? = null): Invoke? {
        if (generation != null && !scanSession.isCurrent(generation)) return null
        val pending = scanSession.finish(generation)
        dismantleCamera()
        scanner?.close()
        scanner = null
        scannerOptions = null
    ...
    private fun configureCamera(formats: List<Int>, generation: Long) {
        activity
            .runOnUiThread {
                if (!scanSession.isCurrent(generation)) return@runOnUiThread
                val vibrator =
    ...
    private fun analyze(image: ImageProxy, generation: Long) {
        ...
        if (!scanSession.isCurrent(generation) || scanner == null || mediaImage == null) {
            image.close()
            return
        }
        try {
            ...
            scanner
                ...
                .addOnSuccessListener(ContextCompat.getMainExecutor(activity)) { barcodes ->
                    if (scanSession.isCurrent(generation) && barcodes.isNotEmpty()) {
                        ...
                        destroy(generation)?.resolve(jsObject)
                    }
                }
            ...
            image.close()
            destroy(generation)?.reject(error.message ?: "Cannot analyze camera image")
        }
    }
    ...
    fun cancel(invoke: Invoke) {
        activity.runOnUiThread {
            // Take the pending scan before cleanup so its Promise is settled.
            destroy()?.reject("cancelled")
            invoke.resolve()
        }
    }
    ...
    fun scan(invoke: Invoke) {
        ...
        activity.runOnUiThread {
            destroy()?.reject("cancelled")
            val generation = scanSession.begin(invoke)
            try {
                if (!hasCamera()) {
                    destroy(generation)?.reject("No camera available on this device")
                } else if (getPermissionState("camera") != PermissionState.GRANTED) {
                    destroy(generation)?.reject("No permission to use camera. Did you request it yet?")
                } else {
                    setupCamera(args.cameraDirection ?: "back", args.windowed, generation)
                    configureCamera(getFormats(args), generation)
                }
            } catch (error: Exception) {
                destroy(generation)?.reject(error.message ?: "Cannot start scanner")
            }
        }
    }
